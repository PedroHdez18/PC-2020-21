...
#Practica 5B. Principio de computadoras
...
do:
	...
	#error_opc:
		...
		li $v0, 4 					#preparamos la escritura por consola
		la $a0, msg_error			#imporimimos el mensaje del error
		syscall 					#llamada al sistema
		j do						#salto al do
	...
	if1:
		...
		li $v0, 4					#preparamos la escritura por consola
		la $a0, result_fun			#escribimos result_fun por consoola
		syscall						#llamada al sistema

		jal fi 						#salto a la etiqueta funcion fi
		mov.s $f12, $f0 			#$f0 -> $f12
		li $v0, 2					#prepara al sistema para imrprimir un flotante
		syscall						#llamada al sistema

		li $v0, 4					#preparamos la escritura por consola
		la $a0, endl				#escribimos un salto de linea
		syscall 					#llamada al sistema
	...
	if2:
		...
		li $v0, 4					#preparamos al sistema para imprimir
		la $a0, result_fun 			#imprimimos result_fun por consola
		syscall						#llamada al sistema
